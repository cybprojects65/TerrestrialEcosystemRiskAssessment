...
# File to select
variables_risk1_standardized <- read.csv(file="./output/Overlap/risk1_overlap_standardized_no_na.csv", header=TRUE, sep=",")
...
multi_centroidi<-  seq(from=5, to=30, by=1) #Number of centroids to test and compare
N <- 1000 # Maximum number of iterations for k-means
...
############# Centroid standard deviation ############# 
  ...
  # Calculation of standard deviations for each cluster
...
#######################################################################################  
  ...
  for (i in 1:nrow(centroide_interpretazione)) {
    if (c_H[i]>c_L[i] & c_H[i]>c_M[i]) {
      centroide_interpretazione[i] <- "closer attention"
    }
  ...
  # Construction of centroids_annotated by inserting columns
  centroidi_annotated <- centroid_id  # Start with ID
  
  for (i in seq_along(feature_names)) {
    num_col <- centroidi_df[[i]]
    sd_col <- centroidi_sd_df[[i]]  # new column for SD
...
########################################################################  
  ...
  write.csv(centroidi_annotated, 
            file = paste0("./output/MultiKmeans/seq_from05_to30_by1_n1000_sd/centroidi_annotated_", n_centroidi, "_", year, ".csv"), 
            row.names = FALSE)
  ...
  output_file<-paste0("./output/MultiKmeans/seq_from05_to30_by1_n1000_sd/centroid_classification_assignment_",n_centroidi,"_", year, ".csv")
