...
variables_risk1_reclassified <- variables_risk1_original %>%
  mutate(
    out003_land_imperviousness_density_change_2024pr = case_when(
      out003_land_imperviousness_density_change_2024pr == 201 ~ 100, # non-sealed in both years (stable non-built up)
      out003_land_imperviousness_density_change_2024pr == 254 ~ NA, # unclassifiable (no satellite image available or or clouds or shadows or or snow)
      out003_land_imperviousness_density_change_2024pr == 255 ~ NA, # outside area
...
# CSV export variables_risk1_reclassified
...
# Standardisation of variables for each column: new value = (value - mean) / sd
...
#--- Step 5. Selection of points without NA values ---####
variables_risk1_standardized_no_na <- variables_risk1_standardized %>%
  filter(complete.cases(.))

#--- Step 6. Calculation of the "overlap index" ---####

risk1_overlap_standardized_no_na <- variables_risk1_standardized_no_na %>%
  # Total risk = sum of the standardised values of the variables, then normalised
  mutate(
    standard_risk_score = rowSums(across(-c(x, y)), na.rm = TRUE), #excludes columns x and y and sums the standardised values
    # add a new column ‘overlap_index_classification’ and assign the values:
    # low risk up to the 50th percentile
    # medium risk from the 50th to the 75th percentile
    # high risk above the 75th percentile
    overlap_index_classification = case_when(
      standard_risk_score <= quantile(standard_risk_score, 0.5, na.rm = TRUE) ~ "low risk",
      standard_risk_score > quantile(standard_risk_score, 0.5, na.rm = TRUE) & standard_risk_score <= quantile(standard_risk_score, 0.75, na.rm = TRUE) ~ "medium risk",
      standard_risk_score > quantile(standard_risk_score, 0.75, na.rm = TRUE) ~ "high risk",
      TRUE ~ NA_character_ # in case of NA values
    )
  )
...
# write.csv(risk1_overlap_standardized, "output/Overlap/risk1_overlap_standardized.csv", row.names = FALSE)
# cat("Export completed.\n")
