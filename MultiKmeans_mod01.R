...
multi_centroidi<-  seq(from=5, to=26, by=1) #c(4)
N <- 1000
...
for (n_centroidi in multi_centroidi){
  ...
  write.csv(centroidi_annotated, 
            file = paste0("./output/all_centroidi_annotated_", n_centroidi, "_", year, ".csv"), 
            row.names = FALSE)
  ...
  output_file<-paste0("./output/all_centroid_classification_assignment_",n_centroidi,"_", year, ".csv")
  ...
  if (length(which(centroid_distribution<=2))>0 || 
    ...
    reference.norm <- rep(1/length(centroid_distribution),length(centroid_distribution))
    ...
    #chisq<-sum((centroid_distribution.norm*1000-reference.norm*1000)^2/(reference.norm*1000))
